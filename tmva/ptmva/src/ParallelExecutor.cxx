...
TMVA::ParallelExecutor::Execute(TMVA::Factory *factory, UInt_t jobs,
                                TMVA::OptionMap options) {
  fWorkers.SetNWorkers(jobs);

  std::vector<std::pair<TString, UInt_t>> methods; // the pair that have the
                                                   // dataset name and the
                                                   // method positon in the
                                                   // vector
  ...
  if (factory->GetAnalysisType() ==
      TMVA::Types::EAnalysisType::kClassification) {
    ...
    auto executor = [factory, methods](UInt_t workerID) -> OptionMap {
      ...
      r["rocint"] = 0;
      r["method"] = factory->GetMethodName(methods[workerID].first,
                                           methods[workerID].second);
      ...
      factory->EvaluateMethod(methods[workerID].first,
                              methods[workerID].second);
      r["rocint"] = factory->GetROCIntegral(methods[workerID].first,
                                            r.GetValue<TString>("method"));
      return r;
    };
    ...
    Log() << kINFO << "-----------------------------------------------------"
          << Endl;
    Log() << kINFO
          << Form("%-20s %-15s %-15s", "Dataset", "Method", "ROC-Integral")
          << Endl;
    Log() << kINFO << "-----------------------------------------------------"
          << Endl;
    auto counter = 0;
    for (auto &item : fResults) {
      Log() << kINFO << Form("%-20s %-15s %#1.3f",
                             item.GetValue<TString>("dataset").Data(),
                             item.GetValue<TString>("method").Data(),
                             item.GetValue<Double_t>("rocint"))
            << Endl;
    ...
    Log() << kINFO << "-----------------------------------------------------"
          << Endl;
    TMVA::gConfig().SetSilent(kTRUE);
    return TMVA::ParallelExecutorResults("ParallelExecutor(Factory)", jobs,
                                         fTimer.RealTime(), fOptResults);
  }

  return TMVA::ParallelExecutorResults("Unknow", jobs, fTimer.RealTime(),
                                       options);
}

//______________________________________________________________________________
const TMVA::ParallelExecutorResults
TMVA::ParallelExecutor::Execute(TMVA::CrossValidation *cv, UInt_t jobs,
                                TMVA::OptionMap options) {
  fWorkers.SetNWorkers(jobs);
  ...
  TMVA::MsgLogger::InhibitOutput();
  TMVA::gConfig().SetSilent(kTRUE);
  return TMVA::ParallelExecutorResults("ParallelExecutor(CV)", jobs,
                                       fTimer.RealTime(), options);
}
