...
public:
   ...
   */
   ParallelExecutorResults(const TString name, UInt_t njobs, Double_t exetime, OptionMap options)
      : TNamed(name.Data(), "ParallelExecutorResults"), fNJobs(njobs), fExecutionTime(exetime), fLogger(name.Data()),
        fOptions(options)
   {
   }
...
public:
   ...
   */
   const ParallelExecutorResults Execute(Configurable *algorithm, UInt_t /*jobs*/,
