...
void TMVA::CrossValidation::EvaluateFold(
    UInt_t fold) // you need to create the folds in dataloader first
{
  TString methodName = fMethod.GetValue<TString>("MethodName");
  TString methodTitle = fMethod.GetValue<TString>("MethodTitle");
  TString methodOptions = fMethod.GetValue<TString>("MethodOptions");
  ...
  TMVA::MethodBase *smethod = fClassifier->BookMethod(
      foldloader, methodName, methodTitle, methodOptions);
  ...
  fResults.fROCs[fold] =
      fClassifier->GetROCIntegral(foldloader->GetName(), methodTitle);
  ...
  TMVA::ResultsClassification *sresults =
      (TMVA::ResultsClassification *)smethod->Data()->GetResults(
