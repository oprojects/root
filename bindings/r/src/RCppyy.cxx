...
typedef std::map<ROOT::R::Cppyy::TCppMethod_t, CallFunc_t *> Method2CallFunc_t;
...
// data ----------------------------------------------------------------------
ROOT::R::Cppyy::TCppScope_t ROOT::R::Cppyy::gGlobalScope = GLOBAL_HANDLE;

// smart pointer types
static std::set<std::string> gSmartPtrTypes = {"auto_ptr", "shared_ptr", "weak_ptr", "unique_ptr"};
...
public:
   ...
   ~ApplicationStarter()
   {
      for (auto ifunc : g_method2callfunc) gInterpreter->CallFunc_Delete(ifunc.second);
   }
} _applicationStarter;
...
static inline TFunction *type_get_method(ROOT::R::Cppyy::TCppType_t klass, ROOT::R::Cppyy::TCppIndex_t idx)
{
   ...
   assert(klass == (ROOT::R::Cppyy::TCppType_t)GLOBAL_HANDLE);
...
static inline ROOT::R::Cppyy::TCppScope_t declaring_scope(ROOT::R::Cppyy::TCppMethod_t method)
{
   ...
   if (m)
      return ROOT::R::Cppyy::GetScope(m->GetClass()->GetName());
   return (ROOT::R::Cppyy::TCppScope_t)GLOBAL_HANDLE;
}

// name to opaque C++ scope representation -----------------------------------
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetNumScopes(TCppScope_t scope)
{
   TClassRef &cr = type_from_handle(scope);
   if (cr.GetClass())
      return 0; // not supported if not at global scope
   assert(scope == (TCppScope_t)GLOBAL_HANDLE);
   return gClassTable->Classes();
}

std::string ROOT::R::Cppyy::GetScopeName(TCppScope_t parent, TCppIndex_t iscope)
{
...
std::string ROOT::R::Cppyy::GetName(const std::string &name)
{
   if (name.size() == 0)
      return name;
...
std::string ROOT::R::Cppyy::ResolveName(const std::string &cppitem_name)
{
...
ROOT::R::Cppyy::TCppScope_t ROOT::R::Cppyy::GetScope(const std::string &sname)
{
...
ROOT::R::Cppyy::TCppType_t ROOT::R::Cppyy::GetTemplate(const std::string & /* template_name */)
{
   return (TCppType_t)0;
}

ROOT::R::Cppyy::TCppType_t ROOT::R::Cppyy::GetActualClass(TCppType_t klass, TCppObject_t obj)
{
   TClassRef &cr = type_from_handle(klass);
...
size_t ROOT::R::Cppyy::SizeOf(TCppType_t klass)
{
   TClassRef &cr = type_from_handle(klass);
   if (cr.GetClass())
      return (size_t)cr->Size();
   return (size_t)0;
}

Bool_t ROOT::R::Cppyy::IsBuiltin(const std::string &type_name)
{
...
Bool_t ROOT::R::Cppyy::IsComplete(const std::string &type_name)
{
...
// memory management ---------------------------------------------------------
ROOT::R::Cppyy::TCppObject_t ROOT::R::Cppyy::Allocate(TCppType_t type)
{
   TClassRef &cr = type_from_handle(type);
   return (TCppObject_t)malloc(cr->Size());
}

void ROOT::R::Cppyy::Deallocate(TCppType_t /* type */, TCppObject_t instance)
{
   free(instance);
}

ROOT::R::Cppyy::TCppObject_t ROOT::R::Cppyy::Construct(TCppType_t type)
{
   TClassRef &cr = type_from_handle(type);
   return (TCppObject_t)cr->New();
}

void ROOT::R::Cppyy::Destruct(TCppType_t type, TCppObject_t instance)
{
   TClassRef &cr = type_from_handle(type);
...
static CallFunc_t *GetCallFunc(ROOT::R::Cppyy::TCppMethod_t method)
{
   ...
   ROOT::R::Cppyy::TCppScope_t scope = declaring_scope(method);
   ...
   if (klass.GetClass() || (func && scope == GLOBAL_HANDLE)) {
      ...
      gInterpreter->CallFunc_SetFuncProto(callf, gcl, func ? func->GetName() : klass->GetName(), callString.c_str(),
                                          func ? (func->Property() & kIsConstMethod) : kFALSE, &offset,
                                          ROOT::kExactMatch);
...
static inline void copy_args(void *args_, void **vargs)
{
   ...
   for (std::vector<TParameter>::size_type i = 0; i < args.size(); ++i) {
      ...
      case 'l': /* long */ vargs[i] = (void *)&args[i].fValue.fLong; break;
      case 'f': /* double */ vargs[i] = (void *)&args[i].fValue.fFloat; break;
      case 'd': /* double */ vargs[i] = (void *)&args[i].fValue.fDouble; break;
      case 'D': /* long double */ vargs[i] = (void *)&args[i].fValue.fLongDouble; break;
      case 'k': /* long long */
      case 'K': /* unsigned long long */
      case 'U': /* unsigned long */
      case 'p': /* void* */ vargs[i] = (void *)&args[i].fValue.fVoidp; break;
      case 'V': /* (void*)type& */ vargs[i] = args[i].fValue.fVoidp; break;
      case 'r': /* const type& */ vargs[i] = args[i].fRef; break;
      default: std::cerr << "unknown type code: " << args[i].fTypeCode << std::endl; break;
      }
   }
}

Bool_t FastCall(ROOT::R::Cppyy::TCppMethod_t method, void *args_, void *self, void *result)
{
...
template <typename T>
static inline T CallT(ROOT::R::Cppyy::TCppMethod_t method, ROOT::R::Cppyy::TCppObject_t self, void *args)
{
   T t{};
...
#define CPPYY_IMP_CALL(typecode, rtype)                                                     \
   rtype ROOT::R::Cppyy::Call##typecode(TCppMethod_t method, TCppObject_t self, void *args) \
   {                                                                                        \
      return CallT<rtype>(method, self, args);                                              \
   }

void ROOT::R::Cppyy::CallV(TCppMethod_t method, TCppObject_t self, void *args)
{
...
void *ROOT::R::Cppyy::CallR(TCppMethod_t method, TCppObject_t self, void *args)
{
   void *r = nullptr;
...
Char_t *ROOT::R::Cppyy::CallS(TCppMethod_t method, TCppObject_t self, void *args)
{
   Char_t *s = nullptr;
...
ROOT::R::Cppyy::TCppObject_t ROOT::R::Cppyy::CallConstructor(TCppMethod_t method, TCppType_t /* klass */, void *args)
{
   void *obj = nullptr;
...
void ROOT::R::Cppyy::CallDestructor(TCppType_t type, TCppObject_t self)
{
   TClassRef &cr = type_from_handle(type);
   cr->Destructor((void *)self, kTRUE);
}

ROOT::R::Cppyy::TCppObject_t ROOT::R::Cppyy::CallO(TCppMethod_t method, TCppObject_t self, void *args,
...
ROOT::R::Cppyy::TCppMethPtrGetter_t ROOT::R::Cppyy::GetMethPtrGetter(TCppScope_t /* scope */, TCppIndex_t /* imeth */)
{
   return (TCppMethPtrGetter_t)0;
}

// handling of function argument buffer --------------------------------------
void *ROOT::R::Cppyy::AllocateFunctionArgs(size_t nargs)
{
   return new TParameter[nargs];
}

void ROOT::R::Cppyy::DeallocateFunctionArgs(void *args)
{
   delete[](TParameter *) args;
}

size_t ROOT::R::Cppyy::GetFunctionArgSizeof()
{
   return sizeof(TParameter);
}

size_t ROOT::R::Cppyy::GetFunctionArgTypeoffset()
{
   return offsetof(TParameter, fTypeCode);
}

// scope reflection information ----------------------------------------------
Bool_t ROOT::R::Cppyy::IsNamespace(TCppScope_t scope)
{
...
Bool_t ROOT::R::Cppyy::IsAbstract(TCppType_t klass)
{
...
Bool_t ROOT::R::Cppyy::IsEnum(const std::string &type_name)
{
   return gInterpreter->ClassInfo_IsEnum(type_name.c_str());
}

// class reflection information ----------------------------------------------
std::string ROOT::R::Cppyy::GetFinalName(TCppType_t klass)
{
...
std::string ROOT::R::Cppyy::GetScopedFinalName(TCppType_t klass)
{
...
Bool_t ROOT::R::Cppyy::HasComplexHierarchy(TCppType_t /* handle */)
{
   // Always TRUE for now (pre-empts certain optimizations).
   return kTRUE;
}

ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetNumBases(TCppType_t klass)
{
...
std::string ROOT::R::Cppyy::GetBaseName(TCppType_t klass, TCppIndex_t ibase)
{
   TClassRef &cr = type_from_handle(klass);
   return ((TBaseClass *)cr->GetListOfBases()->At(ibase))->GetName();
}

Bool_t ROOT::R::Cppyy::IsSubtype(TCppType_t derived, TCppType_t base)
{
   if (derived == base)
      return kTRUE;
...
void ROOT::R::Cppyy::AddSmartPtrType(const std::string &type_name)
{
   gSmartPtrTypes.insert(ResolveName(type_name));
}

Bool_t ROOT::R::Cppyy::IsSmartPtr(const std::string &type_name)
{
   ...
   return gSmartPtrTypes.find(real_name.substr(0, real_name.find("<"))) != gSmartPtrTypes.end();
}

// type offsets --------------------------------------------------------------
ptrdiff_t ROOT::R::Cppyy::GetBaseOffset(TCppType_t derived, TCppType_t base, TCppObject_t address, int direction,
                                        bool rerror)
{
   ...
   offset =
      gInterpreter->ClassInfo_GetBaseOffset(cd->GetClassInfo(), cb->GetClassInfo(), (void *)address, direction > 0);
...
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetNumMethods(TCppScope_t scope)
{
   TClassRef &cr = type_from_handle(scope);
   if (cr.GetClass() && cr->GetListOfMethods()) {
      ROOT::R::Cppyy::TCppIndex_t nMethods = (TCppIndex_t)cr->GetListOfMethods()->GetSize();
...
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetMethodIndexAt(TCppScope_t /* scope */, TCppIndex_t /* imeth */)
{
   return (TCppIndex_t)0;
}

std::vector<ROOT::R::Cppyy::TCppMethod_t> ROOT::R::Cppyy::GetMethodsFromName(TCppScope_t scope, const std::string &name)
{
   ...
   if (scope == GLOBAL_HANDLE) {
      ...
      while ((func = (TFunction *)ifunc.Next())) {
         ...
         if (fn.rfind(name, 0) == 0) {
            ...
            if ((name.size() == fn.size()) || (name.size() < fn.size() && fn[name.size()] == '<')) {
      ...
      if (cr.GetClass()) {
         ...
         if (m)
            methods.push_back((TCppMethod_t)m);
      }
   }
...
ROOT::R::Cppyy::TCppMethod_t ROOT::R::Cppyy::GetMethod(TCppScope_t scope, TCppIndex_t imeth)
{
   TFunction *f = type_get_method(scope, imeth);
   return (ROOT::R::Cppyy::TCppMethod_t)f;
}

std::string ROOT::R::Cppyy::GetMethodName(TCppMethod_t method)
{
   if (method) {
...
std::string ROOT::R::Cppyy::GetMethodResultType(TCppMethod_t method)
{
   if (method) {
...
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetMethodNumArgs(TCppMethod_t method)
{
   if (method)
...
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetMethodReqArgs(TCppMethod_t method)
{
   if (method) {
      TFunction *f = (TFunction *)method;
...
std::string ROOT::R::Cppyy::GetMethodArgName(TCppMethod_t method, int iarg)
{
   if (method) {
      TFunction *f = (TFunction *)method;
...
std::string ROOT::R::Cppyy::GetMethodArgType(TCppMethod_t method, int iarg)
{
   if (method) {
      TFunction *f = (TFunction *)method;
      TMethodArg *arg = (TMethodArg *)f->GetListOfMethodArgs()->At(iarg);
...
std::string ROOT::R::Cppyy::GetMethodArgDefault(TCppMethod_t method, int iarg)
{
   if (method) {
      TFunction *f = (TFunction *)method;
      TMethodArg *arg = (TMethodArg *)f->GetListOfMethodArgs()->At(iarg);
...
std::string ROOT::R::Cppyy::GetMethodSignature(TCppScope_t /* scope */, TCppIndex_t /* imeth */)
{
   return "<unknown>";
}

Bool_t ROOT::R::Cppyy::IsConstMethod(TCppMethod_t method)
{
   if (method) {
      TFunction *f = (TFunction *)method;
...
Bool_t ROOT::R::Cppyy::IsMethodTemplate(TCppMethod_t method)
{
   if (method) {
      TFunction *f = (TFunction *)method;
...
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetMethodNumTemplateArgs(TCppScope_t /* scope */, TCppIndex_t /* imeth */)
{
   return (TCppIndex_t)0;
}

std::string ROOT::R::Cppyy::GetMethodTemplateArgName(TCppScope_t /* scope */, TCppIndex_t /* imeth */,
                                                     TCppIndex_t /* iarg */)
{
   return "<unknown>";
}

ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetGlobalOperator(TCppScope_t /* scope */, TCppType_t /* lc */,
                                                              TCppType_t /* rc */, const std::string & /* op */)
{
   return (TCppIndex_t)0;
}

// method properties ---------------------------------------------------------
Bool_t ROOT::R::Cppyy::IsConstructor(TCppMethod_t method)
{
   if (method) {
      TFunction *f = (TFunction *)method;
...
Bool_t ROOT::R::Cppyy::IsPublicMethod(TCppMethod_t method)
{
   if (method) {
      TFunction *f = (TFunction *)method;
...
Bool_t ROOT::R::Cppyy::IsStaticMethod(TCppMethod_t method)
{
   if (method) {
      TFunction *f = (TFunction *)method;
...
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetNumDatamembers(TCppScope_t scope)
{
   TClassRef &cr = type_from_handle(scope);
   if (cr.GetClass() && cr->GetListOfDataMembers())
      ...
      if (g_globalvars.size() != (GlobalVars_t::size_type)vars->GetSize()) {
         ...
         TGlobal *var = 0;
         while ((var = (TGlobal *)ivar.Next())) g_globalvars.push_back(var);
      }
...
std::string ROOT::R::Cppyy::GetDatamemberName(TCppScope_t scope, TCppIndex_t idata)
{
   TClassRef &cr = type_from_handle(scope);
   if (cr.GetClass()) {
...
std::string ROOT::R::Cppyy::GetDatamemberType(TCppScope_t scope, TCppIndex_t idata)
{
   if (scope == GLOBAL_HANDLE) {
      TGlobal *gbl = g_globalvars[idata];
      std::string fullType = gbl->GetFullTypeName();
      if (fullType[fullType.size() - 1] == '*' && fullType.find("char", 0, 4) == std::string::npos)
         fullType.append("*");
...
ptrdiff_t ROOT::R::Cppyy::GetDatamemberOffset(TCppScope_t scope, TCppIndex_t idata)
{
   if (scope == GLOBAL_HANDLE) {
      TGlobal *gbl = g_globalvars[idata];
...
ROOT::R::Cppyy::TCppIndex_t ROOT::R::Cppyy::GetDatamemberIndex(TCppScope_t scope, const std::string &name)
{
   if (scope == GLOBAL_HANDLE) {
      ...
      TClassRef &cr = type_from_handle(scope);
      if (cr.GetClass()) {
         TDataMember *dm = (TDataMember *)cr->GetListOfDataMembers()->FindObject(name.c_str());
...
Bool_t ROOT::R::Cppyy::IsPublicData(TCppScope_t scope, TCppIndex_t idata)
{
   if (scope == GLOBAL_HANDLE)
      return kTRUE;
   TClassRef &cr = type_from_handle(scope);
...
Bool_t ROOT::R::Cppyy::IsStaticData(TCppScope_t scope, TCppIndex_t idata)
{
   if (scope == GLOBAL_HANDLE)
      return kTRUE;
   TClassRef &cr = type_from_handle(scope);
...
Bool_t ROOT::R::Cppyy::IsConstData(TCppScope_t scope, TCppIndex_t idata)
{
   if (scope == GLOBAL_HANDLE) {
      TGlobal *gbl = g_globalvars[idata];
...
Bool_t ROOT::R::Cppyy::IsEnumData(TCppScope_t scope, TCppIndex_t idata)
{
   if (scope == GLOBAL_HANDLE) {
      TGlobal *gbl = g_globalvars[idata];
...
Int_t ROOT::R::Cppyy::GetDimensionSize(TCppScope_t scope, TCppIndex_t idata, int dimension)
{
   if (scope == GLOBAL_HANDLE) {
      TGlobal *gbl = g_globalvars[idata];
