...
public:
   ...
   template <class Type>
   void Send(const Type &var, Int_t dest, Int_t tag) const;

   template <class Type>
   void Recv(Type &var, Int_t source, Int_t tag) const;

   template <class Type>
   void Recv(Type &var, Int_t source, Int_t tag, TStatus &status) const;

   template <class Type>
   TRequest ISend(const Type &var, Int_t dest, Int_t tag);
   ...
   template <class Type>
   void Bcast(Type &var, Int_t root) const;

   template <class Type>
   TRequest IBcast(Type &var, Int_t root) const;

   template <class Type>
   void Reduce(const Type &in_var, Type &out_var, TOp<Type> (*opf)(), Int_t root) const;
   ...
   template <class Type>
   void Send(const Type *vars, Int_t count, Int_t dest, Int_t tag) const;

   template <class Type>
   void Recv(Type *vars, Int_t count, Int_t source, Int_t tag) const;

   template <class Type>
   void Recv(Type *vars, Int_t count, Int_t source, Int_t tag, TStatus &status) const;

   // methods with nonblocking//

   template <class Type>
   TRequest ISend(const Type *vars, Int_t count, Int_t dest, Int_t tag);
   ...
   template <class Type>
   TRequest IBcast(Type *vars, Int_t count, Int_t root) const;

   template <class Type>
   void Bcast(Type *vars, Int_t count, Int_t root) const;

   template <class Type>
   void Reduce(const Type *in_vars, Type *out_vars, Int_t count, TOp<Type> (*opf)(), Int_t root) const;

   template <class Type>
   void Scatter(const Type *in_vars, Int_t incount, Type *out_vars, Int_t outcount, Int_t root) const;

   template <class Type>
   void Gather(const Type *in_vars, Int_t incount, Type *out_vars, Int_t outcount, Int_t root) const;
   ...
   template <class Type>
   void AllReduce(const Type *in_vars, Type *out_vars, Int_t count, TOp<Type> (*opf)()) const;

   template <class Type>
   void AllGather(const Type *in_vars, Int_t incount, Type *out_vars, Int_t outcount) const;
    ...
    * be the same unserializing
             */
   template <class T>
   static void Serialize(Char_t **buffer, Int_t &size, const T *vars, Int_t count, const TCommunicator *comm,
                         Int_t dest = 0, Int_t source = 0, Int_t tag = 0, Int_t root = 0)
   {
    ...
    * be the same serializing
             */
   template <class T>
   static void Unserialize(Char_t *buffer, Int_t size, T *vars, Int_t count, const TCommunicator *comm, Int_t dest = 0,
                           Int_t source = 0, Int_t tag = 0, Int_t root = 0)
   {
      ...
      if (msgis->data()->GetDataTypeName() != ROOT_MPI_TYPE_NAME(T)) {
         comm->Error(__FUNCTION__, "Error unserializing objects type %s where objects are %s \n", ROOT_MPI_TYPE_NAME(T),
                     msgis->data()->GetDataTypeName().Data());
         comm->Abort(ERR_TYPE);
      }
...
/**
    ...
    */
template <class Type>
void TCommunicator::Send(const Type &var, Int_t dest, Int_t tag) const
{
   Send(&var, 1, dest, tag);
}
...
*/
template <class Type>
void TCommunicator::Recv(Type &var, Int_t source, Int_t tag) const
{
...
*/
template <class Type>
void TCommunicator::Recv(Type &var, Int_t source, Int_t tag, TStatus &status) const
{
...
template <class Type>
void TCommunicator::Recv(Type *vars, Int_t count, Int_t source, Int_t tag) const
{
   if (std::is_class<Type>::value) {
      Int_t size;
      TStatus s;
      ...
      ROOT_MPI_CHECK_CALL(MPI_Recv, ((void *)vars, count, GetDataType<Type>(), source, tag, fComm, MPI_STATUS_IGNORE),
                          this);
   }
}
//______________________________________________________________________________
 ...
 */
template <class Type>
void TCommunicator::Recv(Type *vars, Int_t count, Int_t source, Int_t tag, TStatus &status) const
{
   ...
   if (std::is_class<Type>::value) {
      ...
      ROOT_MPI_CHECK_DATATYPE(Type, this);
      // TODO: added status argument to this method
      ROOT_MPI_CHECK_CALL(MPI_Recv, ((void *)vars, count, GetDataType<Type>(), source, tag, fComm, &status.fStatus),
                          this);
 ...
 */
template <class Type>
TRequest TCommunicator::ISend(const Type &var, Int_t dest, Int_t tag)
{
 ...
 */
template <class Type>
TRequest TCommunicator::ISend(const Type *vars, Int_t count, Int_t dest, Int_t tag)
{
   TRequest req;
   if (std::is_class<Type>::value) {
      Char_t *buffer = NULL;
...
template <class Type>
TRequest TCommunicator::ISsend(const Type &var, Int_t dest, Int_t tag)
{
 ...
 */
template <class Type>
TRequest TCommunicator::ISsend(const Type *vars, Int_t count, Int_t dest, Int_t tag)
{
   TRequest req;
   if (std::is_class<Type>::value) {
      Char_t *buffer = NULL;
      ...
      ROOT_MPI_CHECK_DATATYPE(Type, this);
      ROOT_MPI_CHECK_CALL(MPI_Issend, ((void *)vars, count, GetDataType<Type>(), dest, tag, fComm, &req.fRequest),
                          this);
   }
   return req;
}
 ...
 */
template <class Type>
TRequest TCommunicator::IRecv(Type &var, Int_t source, Int_t tag) const
{
 ...
 */
template <class Type>
TRequest TCommunicator::IRecv(Type *vars, Int_t count, Int_t source, Int_t tag) const
{
   TRequest req;
   if (std::is_class<Type>::value) {
      Int_t size;
      ...
      ROOT_MPI_CHECK_DATATYPE(Type, this);
      ROOT_MPI_CHECK_CALL(MPI_Irecv, ((void *)vars, count, GetDataType<Type>(), source, tag, fComm, &req.fRequest),
                          this);
   }
   return req;
}
...
*/
template <class Type>
void TCommunicator::Bcast(Type &var, Int_t root) const
{
   Bcast(&var, 1, root);
}
 ...
 */
template <class Type>
void TCommunicator::Bcast(Type *vars, Int_t count, Int_t root) const
{
   if (std::is_class<Type>::value) {
      Int_t size;
      Char_t *buffer = NULL;
 ...
 */
template <class Type>
TRequest TCommunicator::IBcast(Type &var, Int_t root) const
{
 ...
 */
template <class Type>
TRequest TCommunicator::IBcast(Type *vars, Int_t count, Int_t root) const
{
...
template <class Type>
void TCommunicator::Scatter(const Type *in_vars, Int_t incount, Type *out_vars, Int_t outcount, Int_t root) const
{
   if (GetRank() == root) {
      if (incount % (GetSize() * outcount) != 0) {
         Fatal("TCommunicator::Scatter",
               "Number of elements sent and elements in receive are not divisible. Can't no split to scatter message");
         Abort(ERR_COUNT);
      }
...
void TCommunicator::Gather(const Type *in_vars, Int_t incount, Type *out_vars, Int_t outcount, Int_t root) const
{
   if (GetRank() == root) {
      ...
      // In scatter is not same because out_vars have not overlapping, I mean I just need to fill the entire vector not
      // a region
...
/**
  ...
  */
template <class Type>
void TCommunicator::Reduce(const Type &in_var, Type &out_var, TOp<Type> (*opf)(), Int_t root) const
{
 ...
 */
template <class Type>
void TCommunicator::Reduce(const Type *in_var, Type *out_var, Int_t count, TOp<Type> (*opf)(), Int_t root) const
{
   auto op = opf();
...
//______________________________________________________________________________
/**
 * Method to apply reduce operation over and array of elements using binary tree reduction. and the results is send to
 * all processes.
 * \param in_var variable to eval in the reduce operation
 * \param out_var variable to receive the variable operation
 * \param count Number of elements to reduce in \p in_var and \p out_var
 ...
 */
template <class Type>
void TCommunicator::AllReduce(const Type *in_vars, Type *out_vars, Int_t count, TOp<Type> (*opf)()) const
{
...
//______________________________________________________________________________
/**
 *  Each process (TCommunicator::GetMainProcess() process included) sends the contents of its send buffer to the
* TCommunicator::GetMainProcess() process.
*  The TCommunicator::GetMainProcess() process receives the messages and stores them in rank order, after that send a
* bcast message with the results.
*  The outcome is as if each of the n processes in the group (including the root process)
 * \param in_vars any selializable object vector reference to send the message
 * \param incount Number of elements in receive in \p in_vars
 * \param out_vars any selializable object vector reference to receive the message
 ...
 */
template <class Type>
void TCommunicator::AllGather(const Type *in_vars, Int_t incount, Type *out_vars, Int_t outcount) const
{
