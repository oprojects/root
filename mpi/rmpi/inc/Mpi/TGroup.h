...
public:
   ...
   inline Bool_t operator==(const TGroup &group) { return (Bool_t)(fGroup == group.fGroup); }
   inline Bool_t operator!=(const TGroup &group) { return (Bool_t) !(fGroup == group.fGroup); }
   ...
   inline operator MPI_Group() const { return (const MPI_Group)fGroup; }
   ...
   static void TranslateRanks(const TGroup &group1, Int_t n, const Int_t ranks1[], const TGroup &group2,
                              Int_t ranks2[]);
